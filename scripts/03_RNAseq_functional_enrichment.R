...
# degResult <- "Tet_5uM_12hr_vs_DMSO_12hr_new"
...
uniqueFgsea <- suppressMessages(readr::read_tsv(file = paste(outPrefix, ".fgsea.tab", sep = ""))) %>% 
  dplyr::select(pathway, DESCRIPTION_BRIEF, everything(), -contrast, contrast)
...
openxlsx::addWorksheet(wb = wb, sheetName = wrkSheet)
openxlsx::writeData(
  wb = wb, sheet = wrkSheet, startCol = 1, startRow = 1,
...
openxlsx::setColWidths(wb = wb, sheet = wrkSheet, cols = 2, widths = 30)
openxlsx::freezePane(wb = wb, sheet = wrkSheet, firstActiveRow = 3, firstActiveCol = 2)
