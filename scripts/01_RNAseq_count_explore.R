...
exptInfo <- suppressMessages(readr::read_tsv(file = file_sampleInfo)) %>% 
  # dplyr::filter(time == "24hr") %>%
  # dplyr::filter(drug %in% c("DMSO", "ADTet")) %>%
  as.data.frame()
...
pltTitle <- paste("Principal Component Analysis (rld):", analysisName)
...
ggsave(filename = paste(outPrefix, ".rld_PCA.pdf", sep = ""), device = "pdf",
       plot = pt_rldPca, width = 10, height = 8)
...
ggsave(filename = paste(outPrefix, ".PCA_scatter.png", sep = ""), device = "png",
       plot = pt_pcScatter, width = 12, height = 10)
